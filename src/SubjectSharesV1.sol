...
contract SubjectSharesV1 is ERC20, Ownable, ReentrancyGuard {
    error PaymentFailed();
    error InvalidInputs();
    ...
    function buyShares(uint112 amount) public payable nonReentrant {
        ...
        if(JOKER_TOKEN.balanceOf(msg.sender) < (paymentAmount + protocolFee + subjectFee)) revert InsufficientPayment();
